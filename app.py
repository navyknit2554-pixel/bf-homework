        ...
        with tab_tt:
            ...
            if not teachers_all:
                ...
                with st.form(f"tt_teacher_form_{sel_teacher['id']}"):
                    ...
                    cell_selections = {}
                    for p in PERIODS:
                        row_bg = "#f8fafc" if p % 2 == 0 else "#ffffff"
                ...
                if preview:
                    ...
                    min_p = min(r["period"] for r in preview)
                    ...
                    for p in range(min_p, max_p+1):
                        ...
                        rc[0].markdown(
                            f"<div style='background:{row_bg};border-left:3px solid #3b82f6;padding:8px;border-radius:4px;color:#1a1a1a;'><b>{p}교시</b></div>",
                            unsafe_allow_html=True)
                        for i, d in enumerate(DAYS):
                            cells = tt_multi.get((d, p), [])
                            if cells:
                                inner = "".join([
                                    f"<div style='background:#e0f2fe;border:1px solid #7dd3fc;border-radius:5px;padding:5px 8px;"
                                    f"text-align:center;font-size:0.8rem;margin-bottom:3px;'>"
                                    f"<b style='color:#0c4a6e;'>{c['grade']} {c['class_name']}</b>"
                                    f"<div style='color:#0369a1;font-size:0.7rem;'>{c['subject']}</div>"
                                    f"</div>"
                                    for c in cells
                                ])
                                ...
                                rc[i+1].markdown(
                                    f"<div style='background:{row_bg};border-radius:5px;padding:6px 8px;text-align:center;color:#94a3b8;font-size:0.8rem;margin:2px;'>—</div>",
                                    unsafe_allow_html=True)
