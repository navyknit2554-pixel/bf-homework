...
# 2) .streamlit/config.toml에 static serving 활성화
_st_dir = Path(".streamlit")
_st_dir.mkdir(exist_ok=True)
_config_path = _st_dir / "config.toml"
_need_static = True
if _config_path.exists():
    _cfg_text = _config_path.read_text()
    if "enableStaticServing" in _cfg_text:
        _need_static = False
if _need_static:
    with open(_config_path, "a") as _cf:
    ...
    if page == "📋 내 과제 목록":
        ...
        if not progress_rows:
            ...
            for r in progress_rows:
                key = f"{r['teacher_subject']} ({r['teacher_name']} 선생님)"
                subj_groups.setdefault(key, []).append(r)

            for subj_label, items in subj_groups.items():
                st.markdown(f"#### 📘 {subj_label}")

                # 과목 전체 진도율
                total_all = sum(it["total_units"] for it in items)
                done_all  = sum(it["completed_units"] for it in items)
                overall_pct = int(done_all / total_all * 100) if total_all > 0 else 0

                bar_color = "#22c55e" if overall_pct >= 80 else "#f59e0b" if overall_pct >= 40 else "#ef4444"
                st.markdown(
                    f"<div style='background:#e2e8f0;border-radius:10px;overflow:hidden;height:28px;margin-bottom:4px;'>"
                    f"<div style='background:{bar_color};height:100%;width:{overall_pct}%;border-radius:10px;"
                    f"display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;font-size:0.85rem;"
                    f"min-width:40px;'>{overall_pct}%</div></div>"
                    f"<div style='text-align:right;font-size:0.78rem;color:#64748b;margin-bottom:12px;'>"
                    f"전체 {done_all} / {total_all} 완료</div>",
                    unsafe_allow_html=True)

                for it in items:
                    pct = int(it["completed_units"] / it["total_units"] * 100) if it["total_units"] > 0 else 0
                    ic = "✅" if pct >= 100 else "🔵" if pct >= 50 else "⬜"
                    col1, col2 = st.columns([3, 1])
                    col1.markdown(f"{ic} **{it['topic']}**")
                    col2.markdown(f"<div style='text-align:right;'><b>{it['completed_units']}</b> / {it['total_units']}</div>", unsafe_allow_html=True)
                    st.progress(min(pct / 100.0, 1.0))
                    if it["note"]:
                        st.caption(f"📝 {it['note']}")
  ...
  <a href="{sms_link}" target="_blank" style="text-decoration:none;">
    <span style="{btn_style}background:#2f9e44;color:white;display:inline-flex;">📱 {label}</span>
        ...
        if not my_students:
            ...
            pm_tab1, pm_tab2, pm_tab3 = st.tabs(["📊 진도 현황", "✏️ 진도 입력", "📝 진도 수정"])

            # ── 진도 현황 ──
            with pm_tab1:
                st.caption("내 학생들의 전체 진도 현황을 확인합니다.")
                conn = get_db()
                ...
                if not all_progress:
                    st.info("아직 등록된 진도 데이터가 없습니다. '진도 입력' 탭에서 추가해주세요.")
                else:
                    # 학생별 그룹핑
                    student_map = {}
                    ...
                    for s_label, items in student_map.items():
                        total_all = sum(it["total_units"] for it in items)
                        done_all  = sum(it["completed_units"] for it in items)
                        overall_pct = int(done_all / total_all * 100) if total_all > 0 else 0
                        bar_color = "#22c55e" if overall_pct >= 80 else "#f59e0b" if overall_pct >= 40 else "#ef4444"

                        with st.expander(f"{'✅' if overall_pct >= 80 else '🔵' if overall_pct >= 40 else '⬜'} {s_label}  —  {overall_pct}%"):
                            st.markdown(
                                f"<div style='background:#e2e8f0;border-radius:8px;overflow:hidden;height:22px;margin-bottom:10px;'>"
                                f"<div style='background:{bar_color};height:100%;width:{overall_pct}%;border-radius:8px;"
                                f"display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;font-size:0.8rem;"
                                f"min-width:36px;'>{overall_pct}%</div></div>",
                                unsafe_allow_html=True)
                            for it in items:
                                pct = int(it["completed_units"] / it["total_units"] * 100) if it["total_units"] > 0 else 0
                                c1, c2, c3 = st.columns([3, 1, 1])
                                c1.write(f"📄 {it['topic']}")
                                c2.write(f"{it['completed_units']} / {it['total_units']}")
                                c3.write(f"**{pct}%**")
                                if it["note"]:
                                    st.caption(f"  └ {it['note']}")
            ...
            # ── 진도 입력 ──
            with pm_tab2:
                st.caption("학생을 선택하고 단원/주제별 진도를 입력합니다.")
                ...
                with st.form("add_progress"):
                    pg_topic = st.text_input("단원/주제 *", placeholder="예) 1단원 음운의 변동, 고전시가 정리 등")
                    col_a, col_b = st.columns(2)
                    pg_total = col_a.number_input("전체 분량", min_value=1, value=100, step=1)
                    pg_done  = col_b.number_input("완료 분량", min_value=0, value=0, step=1)
                    pg_note  = st.text_input("비고", placeholder="메모 (선택)")
                    if st.form_submit_button("📊 진도 등록", type="primary", use_container_width=True):
                        if not pg_topic.strip():
                            st.error("단원/주제를 입력해주세요.")
                        elif pg_done > pg_total:
                            st.error("완료 분량이 전체 분량보다 클 수 없습니다.")
                        else:
                            conn = get_db()
                            try:
                                conn.execute("""
                                    INSERT INTO student_progress (student_id, teacher_id, subject, topic, total_units, completed_units, note)
                                    VALUES (?,?,?,?,?,?,?)
                                """, (sel_student["id"], tid_pm, tinfo_pm["subject"],
                                      pg_topic.strip(), pg_total, pg_done, pg_note.strip() or None))
                                conn.commit()
                                st.success(f"✅ {sel_student['name']} — '{pg_topic.strip()}' 진도 등록 완료!")
                                st.rerun()
                            except sqlite3.IntegrityError:
                                st.error("이미 같은 단원이 등록되어 있습니다. '진도 수정' 탭에서 수정해주세요.")
                            finally:
                                conn.close()

                # 일괄 입력 (같은 단원을 여러 학생에게)
                st.divider()
                st.markdown("**📋 일괄 입력** — 같은 단원을 여러 학생에게 한번에 등록")
                with st.form("bulk_progress"):
                    bp_topic = st.text_input("단원/주제 *", placeholder="예) 2단원 문법 요소", key="bp_topic")
                    col_c, col_d = st.columns(2)
                    bp_total = col_c.number_input("전체 분량", min_value=1, value=100, step=1, key="bp_total")
                    bp_done  = col_d.number_input("초기 완료 분량", min_value=0, value=0, step=1, key="bp_done")
                    bp_note  = st.text_input("비고", placeholder="메모 (선택)", key="bp_note")
                    bp_students = st.multiselect("대상 학생", [f"{s['name']} ({s['grade']} {s['class_name']})" for s in my_students], key="bp_students")
                    if st.form_submit_button("📊 일괄 등록", type="primary", use_container_width=True):
                        if not bp_topic.strip():
                            st.error("단원/주제를 입력해주세요.")
                            ...
                            for s_label in bp_students:
                                ...
                                try:
                                    conn.execute("""
                                        INSERT INTO student_progress (student_id, teacher_id, subject, topic, total_units, completed_units, note)
                                        VALUES (?,?,?,?,?,?,?)
                                    """, (s_obj["id"], tid_pm, tinfo_pm["subject"],
                                          bp_topic.strip(), bp_total, bp_done, bp_note.strip() or None))
                                    cnt_ok += 1
                                except sqlite3.IntegrityError:
                                    cnt_skip += 1
                            conn.commit()
                            conn.close()
                            msg = f"✅ {cnt_ok}명 등록 완료!"
                            if cnt_skip:
                                msg += f" ({cnt_skip}명은 이미 등록되어 건너뜀)"
                            st.success(msg)
                            st.rerun()

            # ── 진도 수정 ──
            with pm_tab3:
                st.caption("등록된 진도를 수정하거나 삭제합니다.")
                s_opts_pm2 = {f"{s['name']} ({s['grade']} {s['class_name']})": s for s in my_students}
                sel_s_pm2 = st.selectbox("학생 선택", list(s_opts_pm2.keys()), key="pm_edit_sel")
                sel_student2 = s_opts_pm2[sel_s_pm2]
                ...
                if not stu_progress:
                    ...
                    for pr in stu_progress:
                        pct = int(pr["completed_units"] / pr["total_units"] * 100) if pr["total_units"] > 0 else 0
                        with st.expander(f"📄 {pr['topic']}  —  {pct}% ({pr['completed_units']}/{pr['total_units']})"):
                            with st.form(f"edit_pr_{pr['id']}"):
                                ep_c1, ep_c2 = st.columns(2)
                                ep_total = ep_c1.number_input("전체", min_value=1, value=pr["total_units"], key=f"ept_{pr['id']}")
                                ep_done  = ep_c2.number_input("완료", min_value=0, value=pr["completed_units"], key=f"epd_{pr['id']}")
                                ep_note  = st.text_input("비고", value=pr["note"] or "", key=f"epn_{pr['id']}")
                                ep_c3, ep_c4 = st.columns(2)
                                if ep_c3.form_submit_button("💾 저장", type="primary", use_container_width=True):
                                    if ep_done > ep_total:
                                        st.error("완료가 전체보다 클 수 없습니다.")
                                    else:
                                        conn = get_db()
                                        conn.execute("""
                                            UPDATE student_progress
                                            SET total_units=?, completed_units=?, note=?, updated_at=datetime('now','localtime')
                                            WHERE id=?
                                        """, (ep_total, ep_done, ep_note.strip() or None, pr["id"]))
                                        conn.commit()
                                        conn.close()
                                        st.success("✅ 저장 완료!")
                                        st.rerun()
                            if st.button("🗑 삭제", key=f"del_pr_{pr['id']}"):
                                conn = get_db()
