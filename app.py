...
# 1) static 디렉토리에 아이콘 생성 (Streamlit static serving 용)
...
# 3) JS로 meta 태그 주입 (static URL 참조 + base64 fallback)
try:
    _pwa_img2 = Image.open("icon.png").convert("RGBA").resize((180, 180), Image.LANCZOS)
    _pwa_buf2 = BytesIO()
    _pwa_img2.save(_pwa_buf2, format="PNG")
    _pwa_b64 = f"data:image/png;base64,{base64.b64encode(_pwa_buf2.getvalue()).decode()}"
except:
    _pwa_b64 = ""
...
(function() {{
    ...
    // Apple Touch Icon — static URL 우선, base64 fallback
    var link = document.createElement('link');
    link.rel = 'apple-touch-icon';
    link.sizes = '180x180';
    link.href = './static/apple-touch-icon.png';
    h.appendChild(link);
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.sidebar:
        ...
        page = st.radio("메뉴", ["🏠 홈","📝 과제 등록","📋 과제 관리","🔍 제출 현황","🎬 영상 관리", q_label, "📢 공지사항"])
        st.divider()
        if st.button("로그아웃", use_container_width=True):
            st.session_state.role = None
    ...
    if page == "🏠 홈":
        ...
        if not today_classes:
            ...
            for c in today_classes:
                ...
                col1.markdown(
                    f"<div style='background:#1e293b;border-left:3px solid #3b82f6;padding:10px 8px;border-radius:4px;text-align:center;'>"
                    f"<span style='font-size:0.95rem;font-weight:bold;'>{c['period']}교시</span><br>"
                    f"<span style='font-size:0.78rem;color:#93c5fd;font-weight:500;'>{time_str}</span></div>",
                    unsafe_allow_html=True)
                col2.markdown(
                    f"<div style='background:#1e3a5f;border-radius:8px;padding:10px 16px;'>"
                    f"<span style='font-size:1rem;font-weight:bold;'>{c['subject']}</span>"
                    f"<span style='color:#94a3b8;font-size:0.85rem;margin-left:12px;'>{c['grade']} {c['class_name']}</span>"
                    f"</div>", unsafe_allow_html=True)
                st.markdown("")
        st.divider()
