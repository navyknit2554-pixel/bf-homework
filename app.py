    ...
    if page == "📋 내 과제 목록":
        ...
        if not progress_with_steps:
            ...
            for subj_label, topic_list in subj_groups.items():
                ...
                # 과목 전체 진도율
                total_steps = sum(len(steps) for _, steps in topic_list)
                done_steps  = sum(sum(1 for s in steps if s["is_done"]) for _, steps in topic_list)
                overall_pct = int(done_steps / total_steps * 100) if total_steps > 0 else 0
                ...
                st.markdown(
                    ...
                    f"전체 {done_steps} / {total_steps} 단계 완료</div>",
                    unsafe_allow_html=True)
                ...
                for pr, steps in topic_list:
                    ...
                    with st.expander(f"{ic} **{pr['topic']}**  —  {done_cnt}/{len(steps)} ({t_pct}%)"):
                        if pr["note"]:
                        ...
                        for step in steps:
                            ...
                            checked = s_col1.checkbox(
                                f"**{step['step_order']}단계** {step['step_name']}",
                                value=bool(step["is_done"]),
                                key=f"stu_step_{step['id']}"
                            )
                            if step["is_done"] and step["done_at"]:
                                s_col2.caption(f"✔ {step['done_at'][:10]}")

                            # 체크 상태 변경 감지 → DB 업데이트
                            if checked != bool(step["is_done"]):
                                conn = get_db()
                                if checked:
                                    conn.execute(
                                        "UPDATE progress_steps SET is_done=1, done_at=datetime('now','localtime') WHERE id=?",
                                        (step["id"],))
                                else:
                                    conn.execute(
                                        "UPDATE progress_steps SET is_done=0, done_at=NULL WHERE id=?",
                                        (step["id"],))
                                conn.commit()
                                conn.close()
                                st.rerun()
        ...
        if not my_students:
            ...
            with pm_tab1:
                ...
                if not all_progress:
                    ...
                    for s_label, topics in student_map.items():
                        total_s = 0
                        done_s  = 0
                        for t in topics:
                            steps = progress_steps_map.get(t["id"], [])
                            total_s += len(steps)
                            done_s  += sum(1 for s in steps if s["is_done"])
                        overall_pct = int(done_s / total_s * 100) if total_s > 0 else 0
                        bar_color = "#22c55e" if overall_pct >= 80 else "#f59e0b" if overall_pct >= 40 else "#ef4444"

                        with st.expander(f"{'✅' if overall_pct >= 80 else '🔵' if overall_pct >= 40 else '⬜'} {s_label}  —  {overall_pct}%  ({done_s}/{total_s})"):
                            st.markdown(
                            ...
                            for t in topics:
                                ...
                                st.markdown(f"**📄 {t['topic']}** — {d_cnt}/{len(steps)} ({t_pct}%)")
                                for s in steps:
                                    mark = "✅" if s["is_done"] else "⬜"
                                    done_info = f"  ({s['done_at'][:10]})" if s["is_done"] and s["done_at"] else ""
                                    st.caption(f"  {mark} {s['step_order']}단계: {s['step_name']}{done_info}")
