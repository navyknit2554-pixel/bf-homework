            ...
            with pm_tab1:
                ...
                if not all_progress:
                    ...
                    for t in topics:
                        ...
                        # 체크 후 rerun 시에도 해당 토글 열린 상태 유지
                        exp_key = f"_exp_open_{t['id']}"
                        is_open = st.session_state.pop(exp_key, False)

                        with st.expander(f"{ic} {t['topic']}  —  {d_cnt}/{len(steps)} ({t_pct}%)", expanded=is_open):
                            st.markdown(
